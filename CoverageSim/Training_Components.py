...
class ResetPublisher:
    def __init__(self):
        ...
        self.node = rclpy.create_node('reset_publisher')
        self.publisher = self.node.create_publisher(Empty, 'reset_uav_topic', 10)

    def publish_reset_command(self):
        msg = Empty()
        self.publisher.publish(msg)
        self.node.get_logger().info('Reset command published')
